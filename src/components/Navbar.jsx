    ...
    <header className="flex flex-wrap sm:justify-start sm:flex-nowrap w-full text-sm py-4">
      <nav className="max-w-[90rem] w-full mx-auto sm:flex sm:items-center sm:justify-between xl:px-1 px-3" aria-label="Global">
        <div className="flex items-center justify-between">
          <a className="flex-none text-xl font-bold text-white">Yogesh</a>
        ...
        <div id="navbar-with-collapse" className={`${isOpen ? 'block' : 'hidden'} transition-all duration-200 overflow-hidden basis-full grow sm:block`}>
          <div className="flex flex-col sm:gap-5 sm:flex-row sm:items-center sm:justify-end sm:mt-0 sm:ps-5">
            ...
            <NavLink className={({ isActive }) => isActive ? navbarClassActive : navbarClass} to='/about'>About Me</NavLink>
            <NavLink className={({ isActive }) => isActive ? navbarClassActive : navbarClass} to='/projects'>Projects</NavLink>
            <NavLink className={({ isActive }) => isActive ? navbarClassActive : navbarClass} to='/contact'>Contact Me</NavLink>
            <NavLink className={({ isActive }) => isActive ? navbarClassActive : navbarClass} to='https://drive.google.com/drive/folders/1m1ydZS4t9GOkjkcqSP8I3sJyhh8dtF2J?usp=sharing' target='_blank'>Resume</NavLink>
          </div>
        </div>
      </nav>
    </header>
