...
const navbarClass = 'px-3 pb-1 text-xl text-stone-50 rounded-lg transition duration-300 ease-in-out hover:bg-gray-800';
const navbarClassActive = 'px-3 pb-1 text-xl text-stone-50 rounded-lg bg-gray-800';

export default function Navbar() {
  return (
    <header className="flex flex-wrap sm:justify-start sm:flex-nowrap w-full text-sm py-4">
      <nav className="max-w-[90rem] w-full mx-auto sm:flex sm:items-center sm:justify-between" aria-label="Global">
        ...
        <div id="navbar-with-collapse" className="hidden transition-all duration-[0.1ms] overflow-hidden basis-full grow sm:block">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-end sm:mt-0 sm:ps-5">
            ...
            <NavLink className={({ isActive }) =>
              isActive ? navbarClassActive : navbarClass
            } to='https://drive.google.com/drive/folders/1m1ydZS4t9GOkjkcqSP8I3sJyhh8dtF2J?usp=sharing' target='_blank'>Resume</NavLink>
          </div>
        </div>
      </nav>
    </header>
